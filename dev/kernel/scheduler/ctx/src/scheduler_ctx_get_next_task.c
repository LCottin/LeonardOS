...
uint32_t scheduler_ctx_get_next_task(void)
{
    ...
    const uint32_t current_task_id = scheduler_ctx_get_current_task();
    const uint32_t next_task_id    = (current_task_id + 1U) % g_p_scheduler_ctx_table->scheduler_nb_tasks;

    return g_p_scheduler_ctx_table->scheduler_tasks_id[next_task_id];
}
