...
static uint64_t            s_mmio_read;          /* Read data from register */
static mmio_io64_read_tc_t s_tc;                 /* Current test case */
...
void run(void)
{
    s_mmio_read = mmio_io64_read((cptr_t)s_mmio_reg_addr);
}

void check(void)
{
    /* Check expected value */
    SPEC_EXPECT_EQ_U64(s_mmio_read, s_tc.expected.expected_value);

    /* Check input value has not been modified */
    SPEC_EXPECT_EQ_U64(s_mmio_reg, s_tc.input.reg_value);
