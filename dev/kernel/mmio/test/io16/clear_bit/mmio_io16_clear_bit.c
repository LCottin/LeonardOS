...
static uint16_t                 s_mmio_reg;              /* Simulated MMIO register */
static addr_t                   s_mmio_reg_addr;         /* Address of simulated MMIO register */
static mmio_io16_clear_bit_tc_t s_tc;                    /* Current test case */
...
void setup(const uint32_t idx)
{
    ...
    s_mmio_reg      = s_tc.input.init_value;
    s_mmio_reg_addr = (addr_t)&s_mmio_reg;
}

void run(void)
{
    for (uint8_t call_idx = 0; call_idx < s_tc.input.number_of_clears; call_idx++)
    {
        mmio_io16_clear_bit(&s_mmio_reg, s_tc.input.bit_index_to_clear[call_idx]);
    }
}
...
void check(void)
{
    ...
    SPEC_EXPECT_EQ_U16(s_mmio_reg, s_tc.expected.expected_result);
