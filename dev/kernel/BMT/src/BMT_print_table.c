...
void BMT_print_table(void)
{
    ...
    {
        const BMT_binary_info_t *p_binary = &g_p_bmt_table->binaries[idx];
        char str[32];
        ...
        boot_print("Binary name: ");
        boot_print(p_binary->name);
        boot_print("\n");
        ...
        boot_print("Binary size: ");
        boot_print(string_utils_itoa(p_binary->binary_size, str, 16));
        memory_ops_set(str, 0, 32);
        boot_print("\n");
        ...
        for (uint32_t seg_idx = 0; seg_idx < p_binary->segments_count; seg_idx++)
        {
            ...
            boot_print("Alignment: ");
            boot_print(string_utils_itoa(p_segment->align, str, 16));
            memory_ops_set(str, 0, 32);
            boot_print("\n");
