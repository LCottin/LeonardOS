...
static mmio_io8_write_tc_t s_tc;                 /* Current test case */
...
void run(void)
{
    mmio_io8_write((ptr_t)s_mmio_reg_addr, s_tc.input.reg_value);
}

void check(void)
{
    /* Check expected value */
    SPEC_EXPECT_EQ_U8(*(uint8_t *)s_mmio_reg_addr, s_tc.expected.expected_value);
