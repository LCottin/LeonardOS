...
static mmio_io8_clear_bit_tc_t s_tc;                    /* Current test case */
...
void run(void)
{
    ...
    {
        mmio_io8_clear_bit(&s_mmio_reg, s_tc.input.bit_index_to_clear[call_idx]);
    }
}
...
void check(void)
{
    ...
    SPEC_EXPECT_EQ_U8(s_mmio_reg, s_tc.expected.expected_result);
