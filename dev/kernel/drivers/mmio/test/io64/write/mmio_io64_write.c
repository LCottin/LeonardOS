...
static mmio_io64_write_tc_t s_tc;                 /* Current test case */
...
void run(void)
{
    mmio_io64_write((ptr_t)s_mmio_reg_addr, s_tc.input.reg_value);
}

void check(void)
{
    /* Check expected value */
    SPEC_EXPECT_EQ_U64(*(uint64_t *)s_mmio_reg_addr, s_tc.expected.expected_value);
