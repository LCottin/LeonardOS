...
class Memory_Table:
    ...
    def generate_overview_table(self) -> str:
        ...
        for binary in self._binaries:
            self._markdown += (f"| {binary.get_name():<16} | {binary.get_type():<4} |             |              |          |             | {binary.get_description():<31} |\n")
            ...
            for section in binary.get_sections():
                ...
                self._markdown += (f"|                  |      | {section.get_name():<11} | {section.get_base_address():<12} | {section.get_size():<8} | {end_address:<11} |   {section.get_description():<29} |\n")
